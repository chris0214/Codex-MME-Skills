...
Key `ray.conf` switches include `SUN_LIGHT_ENABLE`, `SUN_SHADOW_QUALITY`, `IBL_QUALITY`, `FOG_ENABLE`, `MULTI_LIGHT_ENABLE`, `OUTLINE_QUALITY`, `TOON_ENABLE`, `SSDO_QUALITY`, `SSR_QUALITY`, `SSSS_QUALITY`, `BOKEH_QUALITY`, `HDR_EYE_ADAPTATION`, `HDR_BLOOM_MODE`, `HDR_FLARE_MODE`, `HDR_STAR_MODE`, `HDR_TONEMAP_OPERATOR`, `AA_QUALITY`, and `POST_DISPERSION_MODE`.

`Shader/textures.fxsub` defines shared resources such as `ScnMap`, `DepthBuffer`, `FogMap`, `LightMap`, `EnvLightMap`, `MaterialMap`, `OutlineMap`, `SSAOMap`, `PSSM1..4`, `Gbuffer2RT..8RT`, `ShadingMap`, and `SSDOMap`. `MaterialMap` routes PMD/PMX objects to `./materials/material_2.0.fx`; controllers, skyboxes, fog, and lights are routed separately or hidden.
